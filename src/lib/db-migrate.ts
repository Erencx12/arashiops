...
export async function runPhase7Migrations(): Promise<{ ok: boolean; message: string }> {
  const errors: string[] = [];
  ...
  for (const [name, slug] of services) {
    await run(`seed integration ${slug}`, () => sql`
  ...
  await run("integration_credentials", () => sql`
  ...
  if (errors.length > 0) {
    console.warn("[phase7 migration] Some steps had errors:", errors);
  }

  return {
    ok: true,
    message: `Phase 7 migrations applied. ${errors.length > 0 ? `${errors.length} non-fatal warnings.` : "All clean."}`,
  };
}
